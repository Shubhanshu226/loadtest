...
let TestsService = class TestsService {
    ...
    baseDir = path_1.default.join(process.cwd(), 'storage', 'tests');
    testDir(testId) {
        return path_1.default.join(this.baseDir, testId);
    }
    metaPath(testId) {
        return path_1.default.join(this.testDir(testId), 'meta.json');
    }
    async ensureDir(p) {
    ...
    // CREATE TEST
    // ===============================
    async createTest(collectionJson, input) {
        const testId = (0, crypto_1.randomUUID)();
        const dir = this.testDir(testId);
        await this.ensureDir(dir);
        await (0, promises_1.writeFile)(path_1.default.join(dir, 'collection.json'), JSON.stringify(collectionJson, null, 2));
        ...
        await (0, promises_1.writeFile)(path_1.default.join(dir, 'script.js'), script);
        const meta = {
            id: testId,
            status: 'created',
        ...
        await (0, promises_1.writeFile)(this.metaPath(testId), JSON.stringify(meta, null, 2));
        return testId;
    }
    // ===============================
    // GET TEST
    ...
    async getTest(testId) {
        ...
        const summaryPath = path_1.default.join(this.testDir(testId), 'summary.json');
        const perApiPath = path_1.default.join(this.testDir(testId), 'per_api_report.json');
        try {
            const summaryRaw = await (0, promises_1.readFile)(summaryPath, 'utf8');
        ...
        try {
            const raw = await (0, promises_1.readFile)(perApiPath, 'utf8');
            const parsed = JSON.parse(raw);
            if (Array.isArray(parsed))
                meta.perRequestReport = parsed;
            else if (Array.isArray(parsed?.rows))
                meta.perRequestReport = parsed.rows;
        }
        catch {
            // ignore if not ready
        }
    ...
    computePerRequestTimings(summary) {
        try {
            // Prefer our custom metric that is guaranteed to be tagged by api
            const m = summary?.metrics?.api_req_duration ??
    ...
    computePerRequestReport(summary) {
        try {
            ...
            for (const k of Object.keys(dur))
                apis.add(this.extractApiFromSubmetricKey(k) ?? k);
    ...
    extractApiFromSubmetricKey(key) {
        const m = /\bapi:([^,}]+)\b/.exec(String(key));
        return m ? m[1] : undefined;
    }
    findSubmetricKeyByApi(submetrics, api) {
        if (!submetrics)
            return undefined;
    ...
    // UPDATE STATUS
    // ===============================
    async updateStatus(testId, patch) {
        const current = await this.getTest(testId);
        const next = { ...current, ...patch };
        await (0, promises_1.writeFile)(this.metaPath(testId), JSON.stringify(next, null, 2));
    }
    // ===============================
    // RUN TEST
    // ===============================
    async runTest(testId) {
        ...
        const dir = this.testDir(testId);
        const scriptPath = path_1.default.join(dir, 'script.js');
        const summaryPath = path_1.default.join(dir, 'summary.json');
        const logPath = path_1.default.join(dir, 'k6.log');
        const metricsPath = path_1.default.join(dir, 'metrics.ndjson');
        await new Promise((resolve) => {
            const child = (0, child_process_1.spawn)('k6', [
                'run',
                '--vus',
                String(meta.vus),
                '--duration',
            ...
            child.on('close', async (code) => {
                await (0, promises_1.writeFile)(logPath, logs);
                if (code === 0) {
                    try {
                        const report = await this.computePerRequestReportFromNdjson(metricsPath, meta.duration);
                        await (0, promises_1.writeFile)(path_1.default.join(dir, 'per_api_report.json'), JSON.stringify({ rows: report }, null, 2), 'utf8');
                    }
                    catch (e) {
                        this.logger.warn('per_api_report_generation_failed', { testId, error: String(e?.message || e) });
                    }
                }
                await this.updateStatus(testId, {
            ...
            child.on('error', async (err) => {
                await (0, promises_1.writeFile)(logPath, String(err));
                await this.updateStatus(testId, {
                    status: 'failed',
                    finishedAt: new Date().toISOString(),
    ...
    async computePerRequestReportFromNdjson(metricsPath, durationStr) {
        const seconds = this.parseDurationSeconds(durationStr) || 1;
        const raw = await (0, promises_1.readFile)(metricsPath, 'utf8');
        ...
        const ensure = (api) => {
            ...
            if (cur)
                return cur;
            const a = { count: 0, errorCount: 0, sum: 0, min: Infinity, max: 0, samples: [], bytesReceived: 0, bytesSent: 0 };
            byApi.set(api, a);
            return a;
        };
        const pushSample = (a, v) => {
            if (!Number.isFinite(v))
                return;
            a.sum += v;
            a.min = Math.min(a.min, v);
            a.max = Math.max(a.max, v);
            if (a.samples.length < 5000)
                a.samples.push(v);
        };
        for (const line of lines) {
            ...
            if (metric === 'api_reqs')
                a.count += value;
            else if (metric === 'api_errors')
                a.errorCount += value;
            else if (metric === 'api_data_received')
                a.bytesReceived += value;
            else if (metric === 'api_data_sent')
                a.bytesSent += value;
            else if (metric === 'api_req_duration')
                pushSample(a, value);
        }
        const pct = (arr, p) => {
            if (!arr.length)
    ...
    generateK6Script(collection, config) {
        ...
        const emailTemplate = (vars.email || '').trim() || 'user+{{runId}}+{{iteration}}@example.com';
...
`;
    ...
    // FLOW EXTRACTION
    // ===============================
    extractFlows(collection) {
        const flows = [];
        const walk = (items, parent = 'flow_default') => {
    ...
    generateFlow(flow, collectionBearer) {
        return `
function ${flow.name}(user, ctx) {

  ${flow.requests.map((r) => this.generateRequest(r, collectionBearer, flow.name)).join('\n')}
}
`;
    }
    generateRequest(rw, collectionBearer, flowName) {
        ...
        const isCreateTask = (method === 'POST' && urlRaw.endsWith('/tasks')) ||
...
`;
    ...
    extractCollectionBearerAuthValue(collection) {
        ...
        const tokenEntry = bearer.find((e) => String(e?.key || '').toLowerCase() === 'token');
    ...
    ensureVuInEmailTemplate(template) {
        const t = String(template || '').trim();
        if (!t)
            return 'user+{{runId}}+{{vu}}+{{iteration}}@example.com';
        ...
        const at = t.indexOf('@');
        if (at > 0)
            return `${t.slice(0, at)}+{{vu}}${t.slice(at)}`;
        return `${t}+{{vu}}`;
    }
    safeName(name) {
