...
const BASE_URL = "http://localhost:3009/graphql";
const RUN_ID = "run";
const EMAIL_TEMPLATE = "user+{{vu}}@example.com";
const DEFAULT_PASSWORD = "Password@123";
const COLLECTION_BEARER_TEMPLATE = "";
const COLLECTION_VARS = {"baseUrl":"http://localhost:3009/graphql","AUTH_USERNAME":"RCC_USR","AUTH_PASSWORD":"RCC_PWD","basicAuthToken":"UkNDX1VTUjpSQ0NfUFdE","email":"user@example.com","password":"Password@123","otp":"000000","guestId":"","mfaToken":"","authToken":""};
const EXTRACTIONS = [{"match":"/Auth :: Login/i","jsonPath":"$.authToken","var":"authToken"},{"match":"CurrentSession","jsonPath":"accessToken","var":"accessToken"}];
...
function flow_default(user, ctx) {
  ...
  {
    const requestKey = "flow_default :: Login";
    const url = template("{{baseUrl}}", {
      baseUrl: BASE_URL,
      runId: RUN_ID,
      email: user.email,
    ...
    const bodyText = "{\n  \"query\": \"mutation Login($input: LoginInput!) { login(input: $input) { nextStep id userId email name authToken accessToken refreshToken mfaToken qrUrl platform xApiKey } }\",\n  \"variables\": {\n    \"input\": {\n      \"email\": \"{{email}}\",\n      \"password\": \"{{password}}\",\n      \"userType\": \"User\"\n    }\n  }\n}";
    let body = undefined;
    if (bodyText != null) {
  ...
  {
    const requestKey = "flow_default :: Current Session";
    const url = template("{{baseUrl}}", {
      baseUrl: BASE_URL,
      runId: RUN_ID,
      email: user.email,
    ...
    const bodyText = "{\n  \"query\": \"query CurrentSession { currentSession { result } }\",\n  \"variables\": {}\n}";
    let body = undefined;
    if (bodyText != null) {
      const rendered = template(bodyText, {
        baseUrl: BASE_URL,
    ...
    const res = http.request("POST", url, body, {
      headers,
      tags: { api: "flow_default :: Current Session" },
      timeout: '30s',
    });
    api_req_duration.add(res.timings.duration, { api: requestKey });
    api_reqs.add(1, { api: requestKey });
