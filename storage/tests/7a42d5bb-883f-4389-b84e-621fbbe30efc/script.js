...
function Auth(user, ctx) {
  ...
  
  {
    ...
    const res = http.request("POST", url, body, { headers, tags: { api: "Auth :: Register (optional per iteration)" } });
...

  {
    ...
    const res = http.request("POST", url, body, { headers, tags: { api: "Auth :: Login (sets accessToken)" } });
    check(res, { 'status ok': (r) => r.status >= 200 && r.status < 400 });
...
function Tasks(user, ctx) {
  ...
  
  {
    ...
    const res = http.request("GET", url, body, { headers, tags: { api: "Tasks :: List" } });
    check(res, { 'status ok': (r) => r.status >= 200 && r.status < 400 });
...

  {
    ...
    const res = http.request("POST", url, body, { headers, tags: { api: "Tasks :: Create (sets taskId)" } });
    check(res, { 'status ok': (r) => r.status >= 200 && r.status < 400 });
...

  {
    ...
    const res = http.request("PATCH", url, body, { headers, tags: { api: "Tasks :: Toggle Done" } });
    check(res, { 'status ok': (r) => r.status >= 200 && r.status < 400 });
