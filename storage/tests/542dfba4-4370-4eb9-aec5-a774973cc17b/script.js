...
function flow_default(user, ctx) {
  ...
  {
    const requestKey = "flow_default :: 1. Onboarding (Send OTP)";
    ...
    const res = http.request("POST", url, body, {
      headers,
      tags: { api: "flow_default :: 1. Onboarding (Send OTP)" },
      timeout: '30s',
    });
  ...
  {
    const requestKey = "flow_default :: 2. Verify Email (OTP)";
    const url = template("{{baseUrl}}", {
      baseUrl: BASE_URL,
      runId: RUN_ID,
      email: user.email,
    ...
    const headers = {"content-type":"application/json","authorization":"Bearer {{mfaToken}}","x-device-token":"sample-device-token"};
    for (const k in headers) {
      headers[k] = template(headers[k], {
        baseUrl: BASE_URL,
        runId: RUN_ID,
    ...
    const bodyText = "{\n  \"query\": \"mutation VerifyEmail($input: VerifyOnboardingInput!) { verifyEmail(input: $input) { success nextStep authToken refreshToken userId xApiKey id } }\",\n  \"variables\": {\n    \"input\": {\n      \"otp\": \"123456\"\n    }\n  }\n}";
    let body = undefined;
    if (bodyText != null) {
      const rendered = template(bodyText, {
        baseUrl: BASE_URL,
    ...
    const res = http.request("POST", url, body, {
      headers,
      tags: { api: "flow_default :: 2. Verify Email (OTP)" },
      timeout: '30s',
    });
    api_req_duration.add(res.timings.duration, { api: requestKey });
    api_reqs.add(1, { api: requestKey });
